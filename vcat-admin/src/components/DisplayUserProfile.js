...
import { HomeFeedLike, HomeUserFeed, HomeCommentInsert } from '../libraries/dashboard';
...
class DisplayUserProfile extends React.Component {
    ...
    async componentDidMount() {
        ...
        this.setState({ isLoading: true }, () => this.DisplayUserProfile())
    ...
    DisplayUserProfile = async () => {
        ...
        const ResponseActivity = await HomeUserFeed(requestData);
    ...
    renderStartPost() {
        ...
        return FeedData.length > 0 && FeedData.map((feed, i) => {
            ...
            return (
                <div className="start-post mt-3 display-post-main t-l" key={i} onClick={(e) => { Notifications.setFeedViewId(feed.id) }}>
                            ...
                            {['pdf', 'csv', 'txt'].includes(feed.feed_paths.split('.').pop()) ?
                                <div className="insert-post-main dflex align-center mt-4">
                                    <div className='thumbnail-image mb-3 doc-icon-pdf'>
                                        <a href={feed.feed_paths} target='_blank' rel="noopener noreferrer" download><GrDocumentPdf style={{ width: "50px", height: "50px", cursor: "pointer" }} /></a>
                                    </div>
                                </div> : null
                                    || feed.feed_paths.split('.').pop() === 'doc' || feed.feed_paths.split('.').pop() === 'docx' ? <div className="insert-post-main dflex align-center mt-4">
                                    <div className='thumbnail-image mb-3 doc-icon-txt'>
                                        <a href={feed.feed_paths} target='_blank' rel="noopener noreferrer" download><GrDocumentText style={{ width: "50px", height: "50px", cursor: "pointer" }} /></a>
                                    </div>
                                </div>
                                    : null
                            }
...
export default observer(DisplayUserProfile);
